...
export function extractJobPostingHtml(html: string, canonicalUrl: string): ImportedJob {
  ...
  if (!job || structuredDescription.length < 400 || structuredDescription.split(/\s+/).length < 70) throw new Error("The full structured job description was not available; paste the responsibilities and requirements");
  if (job.validThrough && Date.parse(String(job.validThrough)) < Date.now()) throw new Error("This posting has expired");
...
export async function importJobFromUrl(value: string): Promise<ImportedJob> {
  ...
  for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect += 1) {
    if (!new Set(["jobs.lever.co","jobs.ashbyhq.com","job-boards.greenhouse.io","boards.greenhouse.io","www.linkedin.com","linkedin.com","careers.toasttab.com","thatch.com"]).has(url.hostname)) throw new Error("This site is not supported for automatic import; paste the full description");
